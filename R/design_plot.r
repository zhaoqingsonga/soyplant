...
getWaterLaneProtected <- function(blockStructure = "w/23/2r/w/2p/w/2p/23/w") {
  ...
  # 预分配列表，最后用unlist合并（避免c()在循环中反复重分配）
  re_list <- vector("list", length(blockStructure))
  ...
  for (i in seq_along(blockStructure)) {
    if (blockStructure[i] == 'w') {
      ...
      re_list[[i]] <- rep(-111, as.numeric(sub('r', '', blockStructure[i])))
    } else if (blockStructure[i] == 'p') {
      re_list[[i]] <- -9
    } else if (grepl('p', blockStructure[i])) {
      re_list[[i]] <- rep(-9, as.numeric(sub('p', '', blockStructure[i])))
    } else {
      re_list[[i]] <- rep(1, as.numeric(blockStructure[i]))
    }
  }

  re <- unlist(re_list)
  totalColumns <- length(re)
...
makelist <- function(myd = data.frame(num = 1:3, name = c('JD12', 'JD17', '五星1'), rp = c(3, 10, 6))) {
  # 向量化版本：使用rep和sequence直接生成
  result <- rep(as.character(myd[[2]]), myd[[3]])
  number <- sequence(myd[[3]])
...
expand_rows <- function(df, replicate_col = "rows", sequence_col = "seq_id") {
  ...
  n_reps <- df[[replicate_col]]
  idx <- rep(seq_len(nrow(df)), n_reps)
  seq_id <- sequence(n_reps)
  result <- cbind(df[idx, , drop = FALSE], setNames(data.frame(seq_id), sequence_col))
...
plant <- function(fi = designPlot(), seed = makelist(), returnfi = TRUE) {
  ...
  # 预先计算位置索引（行列坐标）
  field_matrix <- fi[, 1:y]
  row_idx_map <- integer(nrow(seed))
  col_idx_map <- integer(nrow(seed))

  for (j in seq_len(nrow(seed))) {
    # 找到值j在矩阵中的位置
    pos <- which(field_matrix == j, arr.ind = TRUE)
    if (nrow(pos) > 0) {
      row_idx_map[j] <- pos[1, 1]
      col_idx_map[j] <- pos[1, 2]
    }
    plantedfi[plantedfi == j] <- paste(seed[j, 1], '|', seed[j, 2], '|', j, sep = "")
  }

  plantedseed$row <- row_idx_map
  plantedseed$col <- col_idx_map
  plantedseed$total_line <- seq_len(nrow(seed))
