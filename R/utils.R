...
increment_generation_name <- function(names, f_vals, max_gen = 9) {
  ...
  if (any(idx_replace)) {
    sub_one <- function(name, f_val) {
      sub("(:\\d+)$", paste0(":", f_val), name, perl = TRUE)
    }
    result[idx_replace] <- mapply(sub_one, names[idx_replace], f_vals[idx_replace] + 1)
  }
  # 处理没有冒号后缀的情况：追加
...
insert_ck_rows <- function(df, ck, first_as_ck = FALSE) {
